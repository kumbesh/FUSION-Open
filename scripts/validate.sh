...
echo "[1/13] Validating Docker Compose and secure HTTP/syslog bind modes..."
...
echo "[2/13] Running collector Vector configuration and VRL unit tests..."
...
echo "[3/13] Validating the Suricata EVE Vector integration configuration..."
...
echo "[4/13] Checking the live common ClickHouse, detection, and correlation schemas..."
...
echo "[5/13] Proving v0.2-to-v0.3 and v0.3-to-v0.4 migrations preserve rows..."
...
echo "[6/13] Proving the v0.4-to-v0.5.2 migrations, evaluation ledger, and scopes are preserving and idempotent..."
fusion_compose exec -T clickhouse clickhouse-client \
...
echo "[7/13] Checking container health..."
for service in clickhouse vector grafana fusion-detection-engine fusion-correlation-engine; do
...
if [ "$skip_samples" = false ]; then
  echo "[8/13] Sending Windows, Linux, security JSON, and TCP/UDP syslog fixtures..."
  ...
  echo "[8/13] Sample ingestion skipped."
fi

echo "[9/13] Verifying v0.1-v0.4 compatibility and normalized telemetry..."
if [ "$skip_samples" = false ]; then
...
echo "[10/13] Validating endpoint and security-source dashboard queries..."
...
echo "[11/13] Running detection engine acceptance tests..."
"$SCRIPT_DIR/validate-detections.sh"

echo "[12/13] Running correlation engine tests and synthetic acceptance smoke..."
"$SCRIPT_DIR/validate-correlations.sh"

echo "[13/13] Checking Grafana, its data source, and all provisioned dashboards..."
...
incident_dashboards=$(curl -fsS -u "$GRAFANA_ADMIN_USER:$GRAFANA_ADMIN_PASSWORD" \
  "$grafana_base/api/search?query=Fusion%20Incidents")
printf '%s' "$incident_dashboards" | grep -q 'fusion-incidents'
provisioned_incident_dashboard=$(curl -fsS -u "$GRAFANA_ADMIN_USER:$GRAFANA_ADMIN_PASSWORD" \
  "$grafana_base/api/dashboards/uid/fusion-incidents")
for title in "Open incidents" "Correlation backlog" "Incidents created over time" "Current incidents (filtered by last seen)" "Selected incident evidence timeline"; do
  printf '%s' "$provisioned_incident_dashboard" | grep -Fq "\"title\":\"$title\""
done
for variable in status severity incident_type host user correlation_rule tactic technique cross_source incident_id; do
  printf '%s' "$provisioned_incident_dashboard" | grep -Fq "\"name\":\"$variable\""
done

echo "Validation passed: v0.1-v0.4 ingestion, v0.5.2 detections, v0.6 correlation/incidents, migrations, restart safety, storage, and all four dashboards are healthy."
